...
// ====== FETCH MOVIES ======
async function fetchMovies(query, targetContainer) {
  try {
    targetContainer.innerHTML = `<p class="text-center text-light">🔍 Loading...</p>`;
    const res = await fetch(`https://www.omdbapi.com/?s=${query}&apikey=${apiKey}`);
    const data = await res.json();

    if (data.Response === "True") {
      displayMovies(data.Search, targetContainer);
    } else {
      targetContainer.innerHTML = `<p class="text-center text-danger">${data.Error}</p>`;
    }
  } catch (error) {
    targetContainer.innerHTML = `<p class="text-center text-danger">⚠️ Something went wrong. Try again!</p>`;
    console.error(error);
  }
}
...
// ====== TRENDING MOVIES ======
const trendingList = ["Inception", "Avatar", "Titanic", "The Dark Knight", "Interstellar", "Avengers", "Frozen", "Gladiator"];

async function loadTrending() {
  trendingMoviesContainer.innerHTML = `<p class="text-center text-light">🔥 Loading Trending Movies...</p>`;
  for (let title of trendingList) {
    await fetchMovies(title, trendingMoviesContainer);
  }
}
...
searchBtn.addEventListener("click", () => {
  ...
  if (query) fetchMovies(query, movieResults);
});
